...
const KnowledgebaseHeader = () => {
  ...
  const { topics } = useDiscover({ id: shq?.knowledgeBaseTopicId! })
  ...
  const title = shq ? shq.webName : topics ? topics.title : "Shq"
  const description = shq
    ? shq.webDescription
    : Object.keys(topics).length !== 0 && topics.description !== undefined
    ? topics.description
    : ""
  const color = shq
    ? shq.appearance?.primaryColor
