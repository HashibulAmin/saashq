...
class TopBar extends React.Component<Props, State> {
  ...
  renderModal() {
    ...
    return (
      <ReactModal
        ...
        ariaHideApp={false}
        className="saashq-modal fade-in"
        overlayClassName="saashq-overlay"
      >
        <h5>{__("Do you want to end this conversation ?")}</h5>
        <div className="modal-footer">
          <button className="saashq-button btn-outline" onClick={this.endConversation}>{__("No")}</button>
          <button className="saashq-button btn-primary" onClick={handleConfirm}>{__("Yes")}</button>
        </div>
      </ReactModal>
    );
  }
  ...
  render() {
    ...
    const topBarClassNames = classNames("saashq-topbar", {
      expanded: isExpanded
    });

    const middleClass = classNames("saashq-middle", "fade-in", {
