...
const createIframe = (setting: Setting) => {
  ...
  // container
  const containerId = `saashq-container-${formId}`;
  const iframeId = `saashq-iframe-${formId}`;
  ...
  // send saashq setting to iframe
  // after iframe load send connection info
  iframe.onload = () => {
    iframe.style.display = 'inherit';

    const handlerSelector = `[data-saashq-modal="${setting.form_id}"]`;
...
setSaasHQProperty('showPopup', (id: string) => {
  postMessageToOne(id, { action: 'showPopup' });
});

setSaasHQProperty('callFormSubmit', (id: string) => {
  postMessageToOne(id, { action: 'callSubmit' });
});

setSaasHQProperty('sendExtraFormContent', (id: string, html: string) => {
  postMessageToOne(id, { action: 'extraFormContent', html });
});

const formSettings = window.saashqSettings.forms || [];
...
window.addEventListener('message', async (event: MessageEvent) => {
  ...
  const { fromSaasHQ, source, message, setting } = data;
  ...
  if (!(fromSaasHQ && source === 'fromForms')) {
    return null;
  }
  ...
  if (message === 'connected') {
    ...
    if (loadType === 'popup') {
      const selector = `[data-saashq-modal="${setting.form_id}"]`;
