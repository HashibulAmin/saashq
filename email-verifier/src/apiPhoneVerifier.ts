...
export const sendFile = async (fileName: string, hostname: string) => {
  try {
    ...
    if (data) {
      const listIds = await getArray('saashq_phone_verifier_list_ids');
...
export const validateSinglePhone = async (phone: string, hostname: string) => {
  ...
  if (response.status === 'success') {
    ...
    debugBase(`Sending single phone validation status to saashq-api`);
...
export const validateBulkPhones = async (
  ...
  if (verifiedPhones.length > 0) {
    try {
      debugBase(`Sending already verified phones to saashq-api`);
...
export const getBulkResult = async (listId: string, hostname: string) => {
  ...
  try {
    ...
    try {
      ...
      debugBase(`Sending bulk phone validation result to saashq-api`);
