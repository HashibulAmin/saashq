...
const bulkTrueMail = async (unverifiedEmails: string[], hostname: string) => {
  ...
  try {
    ...
    if (data) {
      const taskIds = await getArray('saashq_email_verifier_task_ids');
...
export const single = async (email: string, hostname: string) => {
  ...
  if (emailOnDb) {
    ...
    return sendRequest({
      ...
      body: {
        ...
        source: EMAIL_VALIDATION_SOURCES.SAASHQ
      }
    });
  }
  ...
  if (response.status === 'success') {
    ...
    debugBase(`Sending single email validation status to saashq-api`);
...
export const bulk = async (emails: string[], hostname: string) => {
  ...
  if (verifiedEmails.length > 0) {
    try {
      debugBase(`Sending already verified emails to saashq-api`);
...
export const getTrueMailBulk = async (taskId: string, hostname: string) => {
  ...
  debugBase(`Sending bulk email validation result to saashq-api`);
