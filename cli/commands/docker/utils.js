...
const commonEnvs = configs => {
  ...
  return {
    ...
    REDIS_HOST: db_server_address || (isSwarm ? 'saashq-dbs_redis' : 'redis'),
...
const mongoEnv = (configs, plugin) => {
  ...
  let db_name = mongo.db_name || 'saashq';
...
//      `mongo-secondary:${configs.secondary_server_address}`,
  ...
  if (isSwarm) {
    return execCommand(
      'docker stack deploy --compose-file docker-compose-dbs.yml saashq-dbs --with-registry-auth --resolve-image changed'
    );
  }
...
//  }
    ...
    services: {
      coreui: {
        ...
        volumes: [
          ...
          './plugin.js:/usr/share/nginx/html/js/plugin.js',
          './plugin-detail:/usr/share/nginx/html/js/plugin',          
        ],
        networks: ['saashq'],
      },
      'plugin-core-api': {
  ...
  if (configs.installer) {
    ...
    if (!fromInstaller) {
      ...
      if (!configs.db_server_address) {
        host = host.replace('saashq-dbs_rabbitmq', '127.0.0.1');
      }
  ...
  const essyncerJSON = {
    plugins: [
      {
        db_name: configs.mongo.db_name || 'saashq',
        collections: [
          {
            name: 'users',
  ...
  await fs.promises.writeFile(
    ...
    server {
            ...
            index index.html;
            error_log /var/log/nginx/saashq.error.log;
            access_log /var/log/nginx/saashq.access.log;
            location / {
  ...
  if (isSwarm) {
    await execCommand('docker service rm saashq_gateway', true);

    return execCommand(
      'docker stack deploy --compose-file docker-compose.yml saashq --with-registry-auth --resolve-image changed'
    );
  }
...
const update = async ({ serviceNames, noimage, uis }) => {
  ...
  if (uis) {
    log('Restart core ui ....');
    await execCommand(`docker service update --force saashq_coreui`);
  }

  log('Updating gateway ....');
  await execCommand(`docker service update --force saashq_gateway`);
};
...
const restart = async name => {
  ...
  if (['gateway', 'coreui', 'crons'].includes(name)) {
    await execCommand(`docker service update --force saashq_${name}`);
    return;
  }

  await execCommand(`docker service update --force saashq_plugin-${name}-api`);
};
