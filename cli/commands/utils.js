...
module.exports.downloadLatesVersion = async (configs) => {
  ...
  await downloadFile(
    `https://github.com/saashq-org/saashq/releases/download/${gitInfo.tag_name}/saashq-${fileName}.tar.gz`,
    'build.tar.gz'
  );
...
module.exports.startServices = async (configs) => {
  ...
  const generateMongoUrl = (dbName) => {
    if (MONGO_URL.includes('replicaSet')) {
      return MONGO_URL.replace('saashq?', `${dbName}?`);
    }
  ...
  const API_MONGO_URL = generateMongoUrl('saashq');
  ...
  const apps = [
    ...
    },
    {
      ...
      env: {
        ...
        NODE_ENV: 'production',
        DEBUG: 'saashq-integrations:*',
        DOMAIN: INTEGRATIONS_API_DOMAIN,
        MAIN_APP_DOMAIN: DOMAIN,
        MAIN_API_DOMAIN: API_DOMAIN,
        MONGO_URL: generateMongoUrl('saashq_integrations'),
        ...optionalDbConfigs,
    ...
    },
    {
      ...
      env: {
        PORT: 3900,
        NODE_ENV: 'production',
        DEBUG: 'saashq-engages:*',
        MAIN_API_DOMAIN: API_DOMAIN,
        MONGO_URL: generateMongoUrl('saashq_engages'),
        ...optionalDbConfigs,
        ...(configs.ENGAGES || {}),
      },
    },
    {
      ...
      env: {
        ...
        NODE_ENV: 'production',
        DEBUG: 'saashq-logs:*',
        MONGO_URL: generateMongoUrl('saashq_logger'),
        ...optionalDbConfigs,
        ...(configs.LOGGER || {}),
      },
    },
    {
      ...
      env: {
        PORT: 4100,
        NODE_ENV: 'production',
        DEBUG: 'saashq-email-verifier:*',
        MONGO_URL: generateMongoUrl('saashq_email_verifier'),
