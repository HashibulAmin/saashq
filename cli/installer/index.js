      ...
      ch.consume(
        ...
        async msg => {
          if (msg !== null) {
            ...
            // Update configs.json
            execSync(`npm run saashq installer-update-configs ${data.type} ${data.name}`);

            if (data.type === 'install') {
              sendMessage(ch, 'Running up ....');
              execSync(`npm run saashq up -- --fromInstaller`);

              sendMessage(ch, 'Syncing ui ....');
              execSync(`npm run saashq syncui ${data.name}`);

              sendMessage(ch, 'Restarting coreui ....');
              execSync(`npm run saashq restart coreui`);

              sendMessage(ch, 'Waiting for 10 seconds for plugin api....');
              await sleep(10000);

              sendMessage(ch, 'Restarting gateway ...');
              execSync(`npm run saashq restart gateway`);
            }
            ...
            if (data.type === 'uninstall') {
              ...
              execSync(`npm run saashq remove-service saashq_plugin_${data.name}_api`);
