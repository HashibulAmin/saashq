...
class SelectBoards extends React.Component<Props, State> {
  ...
  renderSelect(selectItem) {
    ...
    return (
      <FlexRow key={_id}>
        ...
        <Select
          className="flex-item"
          placeholder={__('Vyberte potrubí')}
          value={pipelineIds}
  ...
  render() {
    return (
      <FormGroup>
        <ControlLabel>{'Desky a Potrubí'}</ControlLabel>
        <p>{__('Do kterých desek chcete přidat tuto skupinu vlastností?')}</p>

        {this.state.selectItems.map((item) => this.renderSelect(item))}
        <br />

        <LinkButton onClick={this.addNew}>
          <Icon icon="plus-1" /> {__('Přidejte další desku')}
        </LinkButton>
      </FormGroup>
    );
  }
