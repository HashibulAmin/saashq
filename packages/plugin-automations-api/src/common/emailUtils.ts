...
export const generateDoc = async ({
  ...
  if (!toEmails?.length) {
    return { error: 'Přijímání e-mailů nebylo nalezeno' };
  }
...
export const handleEmail = async ({
  ...
  if (!params) {
    return { error: 'Při načítání dat se něco pokazilo' };
  }
...
const sendEmails = async ({
  ...
  if (!fromEmail && !COMPANY_EMAIL_FROM) {
    throw new Error('Od E-mail je vyžadováno');
  }
  ...
  for (const toEmail of toEmails) {
    ...
    if (
      ...
      headers = {
        'X-SES-CONFIGURATION-SET': AWS_SES_CONFIG_SET || 'saashq',
        EmailDeliveryId: emailDelivery && emailDelivery._id,
      };
    } else {
      headers['X-SES-CONFIGURATION-SET'] = 'saashq';
    }

    mailOptions.headers = headers;

    if (!mailOptions.from) {
      throw new Error(`Chybí e-mailová adresa "Od": ${mailOptions.from}`);
    }
