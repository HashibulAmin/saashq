...
export const getConfigs = async (models: IModels) => {
  const configsCache = await redis.get('configs_saashq_integrations');
  ...
  await redis.set('configs_saashq_integrations', JSON.stringify(configsMap));
...
export const resetConfigsCache = async () => {
  await redis.set('configs_saashq_integrations', '');
};
