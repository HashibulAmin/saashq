...
export const posInit = async (req, res) => {
  ...
  data.productGroups = await getProductsData(subdomain, models, pos as any);
  data.posId = pos._id;
...
export const posSyncConfig = async (req, res) => {
  ...
  switch (type) {
    case 'config':
      return res.send(await getConfigData(subdomain, pos as any));
    case 'products':
      return res.send({
        productGroups: await getProductsData(subdomain, models, pos as any),
      });
    case 'slots':
      return res.send({
