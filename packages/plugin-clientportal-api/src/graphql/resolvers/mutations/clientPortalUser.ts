import { authCookieOptions, getEnv } from '@saashq/api-utils/src/core';
import { checkPermission } from '@saashq/api-utils/src/permissions';
import { IAttachment } from '@saashq/api-utils/src/types';
...
const clientPortalUserMutations = {
  ...
  clientPortalFacebookAuthentication: async (
    ...
    try {
      ...
      if (customer) {
        qry = { saashqCustomerId: customer._id, clientPortalId };
      }
      ...
      if (customer && customer._id) {
        user.saashqCustomerId = customer._id;
        await models.ClientPortalUsers.updateOne(
          { _id: user._id },
          { $set: { saashqCustomerId: customer._id } },
        );
      }
  ...
  clientPortalUserAssignCompany: async (
    _root,
    args: { userId: string; saashqCompanyId: string; saashqCustomerId: string },
    ...
    const { userId, saashqCompanyId, saashqCustomerId } = args;

    const getCompany = await sendContactsMessage({
      subdomain,
      action: 'companies.findOne',
      data: { _id: saashqCompanyId },
      isRPC: true,
    });

    let setOps: any = { saashqCompanyId };
    ...
    try {
      ...
      await sendCoreMessage({
        ...
        data: {
          mainType: 'customer',
          mainTypeId: saashqCustomerId,
          relType: 'company',
          relTypeId: saashqCompanyId,
        },
      });
    } catch (error) {
