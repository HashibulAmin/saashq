...
export const handleContacts = async (args: IContactsParams) => {
  ...
  if (type === 'company') {
    ...
    if (company) {
      qry = { saashqCompanyId: company._id, clientPortalId };
    }
    ...
    if (company && company._id) {
      user.saashqCompanyId = company._id;
      await models.ClientPortalUsers.updateOne(
        { _id: user._id },
        { $set: { saashqCompanyId: company._id } },
      );
...
export const putActivityLog = async (subdomain, user) => {
  ...
  let contentId = user.saashqCustomerId;

  if (user.type === 'company') {
    contentType = 'contacts:company';
    contentId = user.saashqCompanyId;
  }
