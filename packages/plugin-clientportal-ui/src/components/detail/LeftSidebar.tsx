...
import { __, renderFullName } from '@saashq/ui/src/utils/core';
  ...
  render() {
    ...
    const onClick = () => {
      if (clientPortalUser.type === 'customer') {
        history.push(`/contacts/details/${clientPortalUser.saashqCustomerId}`);
      }
      if (clientPortalUser.type === 'company') {
        history.push(`/companies/details/${clientPortalUser.saashqCompanyId}`);
      }
    };
