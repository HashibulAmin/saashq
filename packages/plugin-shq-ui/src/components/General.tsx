...
export default function General(props: Props) {
  const { kbTopics, shq, edit, getKbCategories, kbCategories } = props;
  const shqFeatures = shq.features || [];

  const [name, setName] = useState(shq.name || '');
  const [description, setDescription] = useState(shq.description || '');
  const [features, setFeatures] = useState(
    shqFeatures.length > 0 ? removeTypename(shqFeatures) : [getEmptyFeature()]
  );
  ...
  const onSave = () => {
    edit({ _id: shq._id, name, description, features });
  };
