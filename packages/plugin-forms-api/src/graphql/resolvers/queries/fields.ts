...
const fieldQueries = {
  ...
  async fields(
    ...
    if (isVisibleToCreate !== undefined) {
      ...
      const saashqDefinedGroup = await models.FieldsGroups.findOne({
        contentType,
        isDefinedBySaasHQ: true,
        code: { $exists: false },
      });

      if (saashqDefinedGroup) {
        groupIds.push(saashqDefinedGroup._id);
      }
    }
...
const fieldsGroupQueries = {
  ...
  async fieldsGroups(
    ...
    let query: any = {
      $or: [{ ...commonQuerySelector }, { isDefinedBySaasHQ: true }],
    };
    ...
    if (isDefinedBySaasHQ !== undefined) {
      query.isDefinedBySaasHQ = isDefinedBySaasHQ;
    }
