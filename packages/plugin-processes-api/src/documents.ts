...
export default {
  ...
  replaceContent: async ({ subdomain, data: { performId, content } }) => {
    ...
    if (
      ...
      results.push(
        '::heads::<script src="https://nmgplugins.s3.us-east-1.amazonaws.com/ebarimt/jquery.js"></script>'
      );
    }

    if (content.includes('{{ seriesBarcode }}')) {
      results.push(
        '::heads::<script src="https://nmgplugins.s3.us-east-1.amazonaws.com/JsBarcode.all.min.js"></script>'
      );
    }

    if (content.includes('{{ seriesQrcode }}')) {
      results.push(
        '::heads::<script src="https://nmgplugins.s3.us-east-1.amazonaws.com/ebarimt/qrcodegen.js"></script>'
      );
    }
