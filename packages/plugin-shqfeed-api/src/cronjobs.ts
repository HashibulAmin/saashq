...
import { FEED_CONTENT_TYPES } from './models/definitions/shq';
...
const generateSendEventData = async (shqList, type, subdomain) => {
  const date = new Date();

  const sendFunction = (shq: any, receivers: any) => {
    sendNotification(subdomain, {
      createdUser: receivers[0],
      title: 'Event notification',
      notifType: 'plugin',
      action: 'event notification',
      content: `${shq.title} is today`,
      link: `/saashq-plugin-shq-feed/list`,
      receivers,
      contentType: 'shqFeed'
    });
    ...
    sendCoreMessage({
      ...
      data: {
        title: 'Event notification',
        body: `${shq.title} is today`,
        receivers
      }
    });
  };

  for (const shq of shqList) {
    if (
      type === 'event' &&
      moment(shq.eventData.startDate).format('MM-DD-YYYY') ===
        moment(date).format('MM-DD-YYYY')
    ) {
      ...
      sendFunction(shq, receivers);
    }

    if (
      type === 'publicHoliday' &&
      moment(shq.createdAt).format('MM-DD-YYYY') ===
        moment(date).format('MM-DD-YYYY')
    ) {
      const receivers = await sendCoreMessage({
        subdomain,
...
export const createCeremonies = async (subdomain: string) => {
  ...
  if (feeds.length > 0) {
    await models.ShqFeed.insertMany(feeds);
  }
...
export const sendCeremonyNotification = async (subdomain: string) => {
  ...
  const eventData = await models.ShqFeed.find({
    contentType: 'event'
  });

  await generateSendEventData(eventData, 'event', subdomain);

  const holidayData = await models.ShqFeed.find({
