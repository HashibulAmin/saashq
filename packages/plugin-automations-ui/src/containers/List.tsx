...
import { DefaultColumnsConfigQueryResponse } from '@saashq/ui-forms/src/settings/properties/types';
      ...
      confirm().then(() => {
        automationsRemove({
          ...
          .then(() => {
            emptyBulk();
            Alert.success(
              'Úspěšně jste smazali automatizaci. Změny budou trvat několik sekund',
              4500,
            );
      ...
      confirm().then(() => {
        automationsArchive({ variables: { automationIds, isRestore } })
          .then(() => {
            emptyBulk();
            Alert.success(
              'Úspěšně jste archivovali automatizaci. Změny budou trvat několik sekund',
              4500,
            );
    ...
    const duplicate = (_id) => {
      confirm().then(() => {
        duplicateMutation({
          ...
          .then(() => {
            Alert.success('Úspěšně jste duplikovali automatizaci.');
          })
          .catch((e) => {
            Alert.error(e.message);
          });
...
export default withProps<Props>(
  ...
  )(withRouter<IRouterProps, any>(ListContainer)),
);
