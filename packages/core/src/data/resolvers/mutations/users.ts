...
import { ILink } from '@saashq/api-utils/src/types';
import { authCookieOptions } from '@saashq/api-utils/src/core';
...
const userMutations = {
  async usersCreateOwner(
    ...
    if (subscribeEmail && process.env.NODE_ENV === 'production') {
      await fetch('https://saashq.org/subscribe', {
        method: 'POST',
        body: JSON.stringify({
          email,
          purpose,
  ...
  async loginWithGoogle(_root, _params, { models, subdomain }: IContext) {
    try {
      ...
      const state = await jwt.sign(
        {
          subdomain,
          redirectUri: `https://${subdomain}.api.saashq.org/api/sso-callback`,
        },
        ...
        const token = await jwt.sign(
          {
            ...
            subdomain,
            redirectUri: `https://${subdomain}.api.saashq.org/api/ml-callback`,
          },
          models.Users.getSecret(),
          { expiresIn: '1d' },
        );
