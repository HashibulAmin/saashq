...
import { DEFAULT_CONSTANT_VALUES } from '@saashq/api-utils/src/constants';
...
const configQueries = {
  ...
  async configsGetVersion(_root, { releaseNotes }) {
    ...
    const saashqDomain = getEnv({ name: 'DOMAIN' });

    const saashqVersion = await fetch(`${saashqDomain}/version.json`).then((r) =>
      r.json(),
    );

    result.version = saashqVersion.packageVersion || '-';

    const response = await fetch(
      `${process.env.CORE_URL || 'https://saashq.org'}/git-release-info`,
    ).then((r) => r.json());
