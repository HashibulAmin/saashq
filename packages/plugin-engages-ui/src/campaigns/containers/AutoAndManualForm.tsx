...
import { ClientPortalConfigsQueryResponse } from '@saashq/plugin-clientportal-ui/src/types';

import { AddMutationResponse } from '@saashq/ui-segments/src/types';
...
import { queries as clientPortalQueries } from '@saashq/plugin-clientportal-ui/src/graphql';
...
const AutoAndManualFormContainer = (props: FinalProps) => {
  ...
  for (const ext of externalIntegrations) {
    const locals = integrations.filter(
      (i) => i._id === ext.saashqApiId && i.isActive,
    );
