    ...
    const apiConversationResponse = await sendInboxMessage({
      ...
      data: {
        ...
        payload: JSON.stringify({
          ...
          integrationId: data.integrationSaasHQApiId,
          content: data?.message?.text,
  ...
  if (!conversationMessage) {
    try {
      ...
      graphqlPubsub.publish(
        `conversationMessageInserted:${conversation.saashqApiId}`,
        {
...
export const receiveMessage = async (req) => {
  ...
  const customer = await createOrUpdateCustomer(models, subdomain, {
    userId,
    oa_id,
    integrationId: integration?.saashqApiId,
    checkFollower: true,
  ...
  await createOrUpdateConversation(models, subdomain, {
    ...
    integrationSaasHQApiId: integration?.saashqApiId,
